...
R_VB=Sigma(3,2)/sqrt((Sigma(2,2)*Sigma(3,3)));
...
fprintf("proportion de contraste de R %f.\n", c_R);
fprintf("proportion de contraste de V %f.\n", c_V);
fprintf("proportion de contraste de B %f.\n", c_B);

[W,D]=eig(Sigma);
[D_trie, indices]=sort(diag(D), 'descend');
W=W(:, indices);
C=(X_c*W);
nv_R=reshape(C(:,1),size(R));
nv_V=reshape(C(:,2),size(V));
nv_B=reshape(C(:,3),size(B));

Sigma_2=(C')*C/n
R_RV=Sigma_2(2,1)/sqrt((Sigma_2(1,1)*Sigma_2(2,2)));
R_RB=Sigma_2(3,1)/sqrt((Sigma_2(1,1)*Sigma_2(3,3)));
R_VB=Sigma_2(3,2)/sqrt((Sigma_2(2,2)*Sigma_2(3,3)));

% Proportions de contraste
c_R=Sigma_2(1,1)/(Sigma_2(1,1)+Sigma_2(2,2)+Sigma_2(3,3));
c_V=Sigma_2(2,2)/(Sigma_2(1,1)+Sigma_2(2,2)+Sigma_2(3,3));
c_B=Sigma_2(3,3)/(Sigma_2(1,1)+Sigma_2(2,2)+Sigma_2(3,3));
fprintf("proportion de contraste de R %f.\n", c_R);
fprintf("proportion de contraste de V %f.\n", c_V);
fprintf("proportion de contraste de B %f.\n", c_B);
