/* attente 27s ou 5 signaux reÃ§us (ex 2.5.3) [PM, le 08/04/13]
 * 
 ...
 *		s'il est assez proche de 3s, c'est qu'il vient d'Ãªtre rÃ©armÃ©, 
 *		"donc" qu'il provient du timer. (bien sur, il y a une petite fenÃªtre 
 *		(normalement < 1 ms) oÃ¹ l'on peut se tromper...)
 *
 */
...
void actif (int sig) { 
	...
	printf ("Toujours actif, restait %ld s, %ld Âµs...\n",duree.it_value.tv_sec,
...
int main () {
	...
	setitimer(ITIMER_REAL, &duree, NULL);  /* armement du timer temps-rÃˆel */
    while ((nbrecus != 5) && (nbalarm != 9)) {
        pause ();
    }
	printf ("reÃ§us %d, alarm %d\n", nbrecus,nbalarm);
    return 0;
}