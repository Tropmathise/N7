...
void addjob(struct liste_job_struct *liste, pid_t pid, int etat, char** cmd) {
    ...
    if (liste->listejobs[max+1]->jobpid == 0) {
        liste->listejobs[max+1]->jobpid = pid;            
        liste->listejobs[max+1]->jobetat = etat;
        liste->listejobs[max+1]->jobid = max + 1;
        for(int j=0; cmd[j]!=0; j++){
            strcat(liste->listejobs[max+1]->cmd, cmd[j]);
            strcat(liste->listejobs[max+1]->cmd, " ");            
        }
        liste->taille=liste->taille+1;
        /*if(cond){
            printf("job [%d] %d '%s' ajouté à la liste des jobs.\n", liste->listejobs[max+1]->jobid, liste->listejobs[max+1]->jobpid, liste->listejobs[max+1]->cmd);
        }*/
    }
}
...
void freecommandejobs(struct liste_job_struct *liste) {
    for (int i=0; i>liste->taille; i++) {
        ...
        free(liste->listejobs[i]);
        liste->listejobs[i]=NULL;
    }
...
void tuer(pid_t pid, struct liste_job_struct *liste){
    for (int i=0; i<liste->taille; i++){
        if (liste->listejobs[i]->jobpid==pid){
            kill(pid,SIGKILL);
        }
