...
% Point de depart de l'algorithme de la puissance iteree : un vecteur
% aleatoire, normalise
v = ones(p,p); v = v/norm(v);
eps = 1e-5;
cv = false;
iv3 = 0;  % pour compter le nombre d iterations effectuees
t_v3 =  cputime; % pour calculer le temps d execution de l'algo
err3 = 0; % residu de la norme du vecteur propre. On sort de la boucle 
% quand err3 <eps 
...
while(~cv)
    ...
    lbda3=v'*AtA*v;
    iv3=iv3+1;
    err3=(abs(lbda3-mu)/abs(mu));
    cv=(err3 <= eps) | (iv3>=kmax);
    %break
    % /!\ Ce break sert a eviter que vous rentriez dans une boucle infini
    % si vous lancez ce script avant de l'avoir modifier. Pensez a le
    % supprimer quand vous coderez la puissance iteree
