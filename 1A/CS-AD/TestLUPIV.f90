...
subroutine facto(m, ldm, n, ipivot, p, q, ierr)
  ...
  for1 : do k=1,n-1
            if (abs(m(k,k))<=eps*norme_f(m,ldm,n)) then 
               write(*,*) "Pivot trop petit"
               ierr =1
               return 
            endif
            m(k+1:n,k) = m(k+1:n,k)/m(k,k)
            m(k+1:n,k+1:n)=m(k+1:n,k+1:n)-dot_product(m(k+1:n,k),m(k,k+1:n))
   end do for1
...
subroutine analyse_erreur(m,ldm,n,b,x,r,xexact)
   ...
   !      Indices de boucles
   integer :: i, j
   double precision :: denominateur, aux
 ...
 !! Calcul et afffichage de la norme du résidu 
   ...
   !r = b
   !call dgemv('n', n, n, -1.D0, m, ldm, x, 1, 1.D0, r, 1) ! attention 1.D0 et pas 1.0
   ...
   ! à déconseiller à cause du lda
   ! résultat différent des deux autres calculs (?!)
   !r = matmul(m,x) - b
   ...
   do i = 1, n
     aux = abs(b(i))
     do j = 1, n
       aux = abs(m(i,j))*abs(x(j)) + aux
     end do
     denominateur = denominateur + aux**2
   end do
   denominateur = sqrt(denominateur)
   write(*,*) norme(r,n), norme_f(m, n, n), norme(x,n), norme(b,n), denominateur
   ...
   write(6,*) " =============================================="
   write(*,*) "Residu equilibre (norme de Frobenius)", norme(r, n)/(norme_f(m, n, n)*norme(x,n) + norme(b,n))
   if(denominateur.gt.sqrt(epsilon(denominateur))) then
     write(*,*) "Residu equilibre", norme(r, n)/denominateur
   else
     write(*,*) "norme du Residu", norme(r, n)
   end if
 
   write(*,*) "Erreur directe", norme(x - xexact, n) / norme(xexact, n)
   write(6,*) " =============================================="
