...
function fct_advection(signal, scheme, Nt, Nx)
...
%  Nx : Nx+1 pas d'espace
    ...
    %%%%%%%%%%%%%%%%%%%%%%%
    A=eye(Nx,Nx);
    A=A+(lambda/2)*(diag(ones(Nx-1,1),1)+diag(-ones(Nx-1,1),-1));
    ...
    for k = 1:Nt+1
        ...
        switch scheme
            case 'explicit'
                for j=2:Nx+1
                    u_np1(j)=(1-lambda)*u_n(j)+lambda*u_n(j-1);
                end   
            case 'implicit'
                u_np1=inv(A)*u_n;
            case 'LaxWendroff'
                for j=2:Nx+1
                    u_np1(j)=u_n(j)-lambda/2*(u_n(j+1)-u_n(j-1))+lambda^2/2*(u_n(j+1)-2*u_n(j)+u_n(j-1));
                end
        end
