...
// ---------------
	...
	public MorpionSwing(ModeleMorpion modele) {
		...
		// Ajouter les elements de la vue 
		Container contenu = fenetre.getContentPane();
		contenu.setLayout(new GridLayout(4,3));
		for (JLabel[] labels : cases) {
			for(JLabel label : labels) {
				contenu.add(label);
			}
		}
		contenu.add(boutonNouvellePartie, BorderLayout.EAST );
		contenu.add(joueur, BorderLayout.CENTER);
		contenu.add(boutonQuitter, BorderLayout.WEST);
...
// ---------------------
	...
	private class ActionJouer extends MouseAdapter {
		@Override
        public void mouseClicked(MouseEvent ev) {
            ((JLabel) ev.getSource()).setIcon(images.get(modele.getJoueur()));
        }
	}
	...
	private class ActionCliquer extends MouseAdapter {
		...
		@Override
		public void mouseClicked(MouseEvent ev){
			JLabel cellule = (JLabel) ev.getSource();
		}
	}
