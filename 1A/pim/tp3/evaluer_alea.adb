...
procedure Evaluer_Alea is
    ...
    is
        ...
        type Tableau_Entier is array (1..Borne) of Integer;
        type Tableau is
            record
        ...
        Frequence : Tableau;
        nb : Integer; --nombre tiré aléatoirement

    begin
        Max := 0;
        Min := Taille;
        Frequence := ((others => 0),Borne);
        Moyenne := Float (Taille)/Float(Borne);
        for i in 1..Taille loop
            Get_Random_Number(nb);
            Frequence.Elements(nb) := Frequence.Elements(nb) + 1;
        end loop;

        for i in 1..Borne loop
            if Frequence.Elements(i)>Max then
                Max:=Frequence.Elements(i);
            else null;
            end if;
            if Frequence.Elements(i)<Min then
                Min := Frequence.Elements(i);
            else null;
            end if;
        end loop;
