...
procedure Exemples_Memoire_Dynamique is
	...
	begin
		...
		Ptr1.all := 5;
                Put_Line ("Ptr1.all = " & Integer'Image (Ptr1.all));
                --Free(Ptr1); sans le free, il y a une erreur
	...
	begin
		...
		-- XXX Que se passe-t-il si on exécute le programme avec valkyrie ? Deux erreurs read et write
		--
	...
	begin
		...
		Free (Ptr);
	end Illustrer_Pointeur_In;  --juste besoin du in car l'adresse n'est pas modifiée
