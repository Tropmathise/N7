...
// pool fork-join
    ...
    protected int[] compute() {
        ...
        if(taille > TF.seuil){
            taille = taille/2;
            TraiterFragment tf1 = new TraiterFragment(tableau, début, début + taille -1);
            TraiterFragment tf2 = new TraiterFragment(tableau, début+taille, fin);
            tf1.fork();
            tf2.fork();
            int[] resTri1=tf1.compute();
            int[] resTri2=tf2.join();
            resTri=TF.fusion(resTri1,resTri2);
        } else {
...
//-------- Pool Fixe
    ...
    throws InterruptedException, ExecutionException {
        ...
        int[] resTriINT= résultats.get(0).get();
        ...
        for(int i=0; i<nbT; i++){
            ...
            résultats.add(xs.submit(new TriLocal(t, d, f)));
        }
        //récupérer et fusionner les résultats.
        for (int i=1;i<nbT;i++){
            resTriINT = TF.fusion(resTriINT, résultats.get(i).get());
        }
		return resTriINT; 
    }
