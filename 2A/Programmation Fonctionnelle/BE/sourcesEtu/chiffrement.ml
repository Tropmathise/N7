...
struct
  ...
  let get_branche l ac = match ac with
    | Lettre _ -> None
    | Noeud(f) ->
          match List.assoc_opt l f with
            | Some a -> Some a
            | None -> None
 ...
 *)
  ...
  let dechiffrer ls ac =
    let rec dechiffrer_e ls ac acc =
      match ls with
        | [] -> []
        | tete::q -> match (get_branche tete ac) with
                    | Some a ->
                      begin
                      match a with 
                        | Lettre le -> le::(dechiffrer_e q acc acc)
                        | Noeud(l) -> dechiffrer_e q (Noeud(l)) acc
                      end
                    | None -> raise CodeNonValide
        in dechiffrer_e ls ac ac
 ...
 *)
  ...
  (*let chiffrer l arbre = match (l,arbre) with 
    | [],_ -> []
    | _ -> *)
